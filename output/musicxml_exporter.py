...
NOTE_LABEL_TO_MXL = {
    ...
    # MICRO key names from tuning.py (generated by _label_from_abs)
    # These MUST come before "Eb" and "Bb" entries or the startswith fallback
    # incorrectly matches the full-flat alter=-1.0 instead of quarter-flat -0.5
    "Eb_half_rast":   ("E", -0.5),
    "Eb_half_bayati": ("E", -0.5),
    "Eb_half_sikah":  ("E", -0.5),
    "Bb_half_rast":   ("B", -0.5),
    "Bb_half_bayati": ("B", -0.5),
    "Bb_half_sikah":  ("B", -0.5),
...
def freq_to_octave(freq_hz: float, step: str = None) -> int:
    """
    ...
    For C notes, applies a tolerance so that calibration shifts (up to ±50 cents)
    don't push a C4 into octave 3.
    """
    if freq_hz <= 0:
        return 4
    ...
    # C notes sit exactly at the octave boundary. A small calibration shift
    # (e.g. -40 cents) can push C4 (261 Hz) just below the boundary into
    # octave 3. Detect this: if the note is a C and the frequency is within
    # 50 cents below the next octave boundary, bump the octave up.
    if step == "C":
        cents_in_octave = (semitones_from_c0 % 12) * 100
        if cents_in_octave > 1150:
            octave += 1
...
def label_to_mxl_note(note: QuantizedNote) -> Optional[MXLNote]:
    ...
    octave = freq_to_octave(note.median_freq, step=step)
